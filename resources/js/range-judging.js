...
var scoreModal = '<div class="modal fade" id="judge_scoring" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="judge_scoringTitle" aria-hidden="true">\
              <div class="modal-dialog modal-dialog-centered" role="document">\
                <div class="modal-content">\
                  ...
                  <small class="text-muted pl-3"><i>Use <b>#</b> button to convert range input into number input.</i></small>\
                  ...
                  <div class="modal-footer">\
                    <span id="validate-input" class="text-danger"></span>\
                    <button type="submit" class="btn btn-primary" id="save">Next</button>\
                  </div>\
                </div>\
              </div>\
            </div>'
  ...
  function () {
    ...
    $('#btnStart').click(function(){
        ...
        success : function(data, textStatus, jqXHR) {
          ...
          maxRange = data['criteria'][0].max_range;
                  ...
                  $('#judge_scoring #criteria-body-'+i+'').append('<div class="row">\
                                                            ...
                                                            <div class="col-10">\
                                                              <div class="form-group">\
                                                                  <input type="text" class="form-control" id="criteria-'+j+'" value="" data-min="0" data-max="'+maxRange+'"/>\
                                                              </div>\
                                                            </div>\
                                                            <div class="col-2">\
                                                              <button id="toDigit" data-input-id="'+j+'" class="btn btn-danger"><i class="fa fa-hashtag"></i></button>\
                                                            </div>\
                                                          </div>');
                  j++;
                  c++;
    ...
    $(document).on('click', '#toDigit', function(e){
        e.preventDefault();
        id = $(this).attr('data-input-id');
      $('#criteria-'+id).data("ionRangeSlider").destroy();
    ...
    $(document).on('click', '#save', function(e){
        ...
        $('#validatemsg').remove();
        if ($('#judge_scoring input').val() == 0) {
          $('#validate-input').append('<span id="validatemsg">Zero is unacceptable</span>');
          validated = false;
        }
